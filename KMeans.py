...
class KMeans():
    ...
    def fit(self, X):
        ...
        ####################
        for j in range(n):
            self.centers[:,j] = np.random.uniform(low=np.percentile(X[:,j], 10), 
        ...
        for iter in range(self.num_iter):
            ...
            ####################
            distances = np.zeros((m, self.k)) # (100, 3)
            for i in range(m):
                for j in range(self.k):
                    diffVec = X[i] - self.centers[j]
                    dist = np.linalg.norm(diffVec, ord=self.order)
                    distances[i,j] = dist
            ...
            for idx in range(self.k):
                cluster_coordinates = X[cluster_idx == idx]    #(selected_samples, 10)
                if self.order == 2:
            ...
            ####################
            if all(cluster_idx == self.cluster_idx):
                print(f"Early Stopped at Iteration {iter}")
                return self
